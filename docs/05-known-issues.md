...
no `try`/`finally`, so any exception — a malformed APK crashing a worker, a pickling
...
- `requirements.txt` was missing `dash`, `dash-bootstrap-components`, `Flask-Login` and
  `psutil`, all imported by the app — added.
...
`time.sleep(200)` between attempts, with no backoff, jitter, or deadline — a worst case
of roughly four hours *for a single APK*, inside a blocking Dash callback.
...
at a time**, verifying output is unchanged at each step — these functions produce
...
removing files is a maintainer's call, not a contributor's — but they mislead anyone
