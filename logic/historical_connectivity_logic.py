...
def process_apks(n_clicks, api_key, start_date, end_date, package_list_input, desired_versions, highlight_config, num_cores, parser_selection, session_id=None):
    ...
    """
    # Apply config overrides (environment variables can override UI settings)
    ...
    # Skip validation every time, just check at runtime
...
# TODO: improve basic check of CSV integrity
...
def process_package(package_name, base_directory, apikey, db_path, start_date, end_date, desired_versions, highlight_config, num_cores, parser_selection, session_id=None):
    ...
    # Download the required APKs (with timeout)
    logger.info(f"Downloading APKs for {package_name}")
    ...
    # Check for cancellation or no downloads
    ...
    # Process the downloaded APKs (no timeout)
    ...
    # Plot the data for each data type (no timeout)
    figs = {}
    for data_type in ['urls', 'subdomains', 'domains']:
        # Check only for cancellation
        if session_id and session_should_cancel(session_id):
...
def download_apks(package_names, apikey, universal_cache_dir, db_path, start_date, end_date, desired_versions, session_id=None):
    ...
    for package_name in package_names:
        ...
        # If we have fewer or equal APKs than desired, use all of them
        if len(sha256_vercode_vtscandate_list) <= desired_versions:
            selected_apks = sha256_vercode_vtscandate_list
            logger.info(f"Using all {len(selected_apks)} available versions (fewer than requested {desired_versions})")
        else:
            # Calculate indices for evenly spaced samples
            indices = np.linspace(0, len(sha256_vercode_vtscandate_list)-1, desired_versions, dtype=int)
            selected_apks = [sha256_vercode_vtscandate_list[i] for i in indices]
            logger.info(f"Selected {len(selected_apks)} evenly spaced versions from {len(sha256_vercode_vtscandate_list)} available")
        
        # Add all selected APKs to download tasks
    ...
    # Download each APK sequentially to have better control over cancellation with Dash
    results = []
    ...
    for i, task in enumerate(download_tasks):
        ...
        if time.time() - start_time > PROCESSING_TIMEOUT:
            logger.warning(f"Time limit ({PROCESSING_TIMEOUT}s) reached after downloading {len(results)}/{len(download_tasks)} APKs")
        ...
        if result:
            ...
            successful_downloads.append(task)  # Keep track of successful downloads
    
    # Update apk_log to only include successfully downloaded APKs
    for package_name in package_names:
    ...
    if len(results) < desired_versions:
        logger.warning(f"Only downloaded {len(results)}/{desired_versions} desired versions in {elapsed_time:.2f} seconds")
    ...
    # Even if we hit the timeout, return what we have
...
def plot_data(all_data, package_name, highlight_config, data_type):
    ...
    # Example saving text from chart
    '''
